...
export default function HealthAlertsScreen() {
  ...
  async function ensureNotificationsEnabled() {
    ...
    if (!granted) {
      Alert.alert(
        'Permission required',
        'Please allow notifications to use optional wellness reminders.'
      );
      return false;
    }
  ...
  async function scheduleDailyMealAlarm(
    ...
    try {
      ...
      Alert.alert('Reminder set', `${title} reminder scheduled successfully.`);
    } catch (error) {
  ...
  async function scheduleWaterAlarm() {
    try {
      ...
      Alert.alert('Water reminder set', 'You will receive reminders every hour.');
    } catch (error) {
