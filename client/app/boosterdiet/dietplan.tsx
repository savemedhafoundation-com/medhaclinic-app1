...
export default function BoosterDietScreen() {
  ...
  useEffect(() => {
    ...
    async function loadPlan() {
      const localPlan = createLocalDietPlan(input);
      ...
      try {
        ...
        const result = await fetchPersonalizedDietPlan(
          requestInput,
          user
        );
        ...
        setState({
          errorMessage:
            error instanceof Error
              ? error.message
              : 'Could not create your wellness support plan.',
          loading: false,
          outputReference: null,
          plan: localPlan,
          source: 'template',
  ...
  const planPreview = useMemo(() => {
    ...
    return [
      state.plan.title,
      state.plan.subtitle,
      state.plan.focusSummary,
      ...state.plan.mealCards.map(card => `${card.name}: ${card.foods.join(', ')}`),
      state.plan.caution,
    ].join('\n');
  }, [state.plan]);
      ...
      <SafeAreaView edges={['bottom']} style={styles.safeArea}>
        ...
        >
          <View style={styles.heroCard}>
            <View style={styles.heroCopy}>
              ...
              <Text style={styles.heroTitle}>
                {state.plan?.title ?? '30-Day Wellness Support Plan'}
              </Text>
              <Text style={styles.heroSubtitle}>
                {state.plan?.subtitle ??
                  'We are building a wellness support plan from your lifestyle, food preferences, and recent wellness context.'}
              </Text>
            </View>
          ...
          <View style={styles.metaCard}>
            <Text style={styles.generatedForText}>
              Built from wellness inputs for {patientName}
            </Text>
          ...
          {state.loading ? (
            <View style={styles.loadingCard}>
              ...
              <Text style={styles.loadingTitle}>Creating your wellness plan...</Text>
              <Text style={styles.loadingSubtitle}>
                Medha Wellness is turning your inputs into general wellness support.
              </Text>
            </View>
          ) : null}
          ...
          {!state.loading && state.plan ? (
            <>
                ...
                <Text
                  style={[
                    ...
                    state.source === 'ai'
                      ...
                      : styles.sourceBadgeTextTemplate,
                  ]}
                >
                  {state.source === 'ai'
              ...
              <ReportAiOutputButton
                contentPreview={planPreview}
                outputReference={state.outputReference}
                screenName="wellness_support_plan"
              />
