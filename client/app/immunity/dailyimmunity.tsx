...
const API_FIELD_MAP: Record<
  QuestionId,
  Exclude<
    keyof SubmitDailyImmunityVariables,
    "immunityScore" | "immunityLevel" | "submittedAt"
  >
> = {
...
const SAVE_CONFIRMATION_ATTEMPTS = 4;
const SAVE_CONFIRMATION_DELAY_MS = 1500;
...
export default function DailyImmunityCheck() {
  ...
  const confirmDelayedSave = async (startedAt: Date) => {
    for (let attempt = 0; attempt < SAVE_CONFIRMATION_ATTEMPTS; attempt += 1) {
      if (attempt > 0) {
        await delay(SAVE_CONFIRMATION_DELAY_MS);
      }

      try {
        const latest = await getLatestDailyImmunitySummary({
          allowFallback: false,
          timeoutMs: 4_000,
        });
        ...
        const submittedAt = new Date(latest.submittedAt);

        if (
          Number.isFinite(submittedAt.getTime()) &&
          submittedAt >= startedAt &&
          latest.immunityScore === assessment.roundedScore &&
          latest.immunityLevel === immunityMeta.apiLevel
        ) {
          return true;
        }
      } catch (confirmationError) {
        console.log("Daily immunity delayed save confirmation failed:", confirmationError);
      }
    }
  ...
  const openSummaryScreen = ({
    ...
    const authoritativeAssessment = assessment;
  ...
  const handleGetResult = async () => {
    ...
    try {
      ...
      console.log("Daily immunity save failed; showing local result:", error);
      const message =
      ...
      if (isTimeout && (await confirmDelayedSave(saveStartedAt))) {
        openSummaryScreen({
          apiSaved: true,
        });
        return;
      ...
      const saveErrorMessage = isTimeout
        ? "Saving is taking longer than expected. This result is shown now, but it may not appear in saved history until you try again or the backend finishes responding."
        : message;
