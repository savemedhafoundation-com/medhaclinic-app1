...
type BackendDailyImmunityHistoryResponse = {
  success?: boolean;
  items?: DailySubmission[];
  data?: {
...
function readDailyImmunityHistoryFromBackend(
  payload: BackendDailyImmunityHistoryResponse
) {
  return payload.data?.submissions ?? payload.items ?? [];
}

async function getDailyImmunityHistoryFromBackend(limit = 180) {
  const response = await requestBackend<BackendDailyImmunityHistoryResponse>(
    `/v1/immunity/history?limit=${limit}`
  );

  return readDailyImmunityHistoryFromBackend(response);
}
...
export async function getLatestDailyImmunitySummary(options?: {
